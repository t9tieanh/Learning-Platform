...
const Chat = () => {
  ...
  return (
    <div className='flex h-screen overflow-hidden min-h-0'>
      ...
      >
        <ConversationList selected={selectedConversation} onSelect={handleSelect} desiredPeerId={peerIdParam} />
      </div>

      {/* Chat area */}
      <div className={`flex-1 h-full min-h-0 overflow-hidden ${selectedConversation ? 'block' : 'hidden'} md:block`}>
        <ChatArea
