...
export default function GlobalChatNotifier() {
    ...
    return (
        <div className="fixed bottom-4 right-4 z-[1000] space-y-3 pointer-events-none">
            {toasts.map(t => (
                <div key={t.id} className="pointer-events-auto w-80 bg-white border border-slate-200 shadow-lg rounded-lg p-3 flex gap-3 animate-in fade-in slide-in-from-bottom-2">
                    {t.avatar && (
                    ...
                    <button onClick={t.onClick} className="text-blue-600 text-sm font-medium hover:underline ml-2">Mở</button>
                </div>
            ))}
        </div>
    )
