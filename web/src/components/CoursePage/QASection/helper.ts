...
export function useQASection(lessonId: string): QASectionHook {
    ...
    const handleAddReply = async (commentId: string) => {
        ...
        try {
            ...
            setComments((prev) => prev.map((c) => (c.id === commentId ? { ...c, replies: c.replies.filter((r) => r !== reply) } : c)))
        } finally {
            setIsSubmitting(false)
            setReplyContent('')
            setReplyingTo(null)
    ...
    const handleLike = (commentId: string, replyId?: string) => {
        setComments((prev) => prev.map((comment) => {
            if (comment.id === commentId) {
                if (replyId) {
                    return { ...comment, replies: comment.replies.map((r) => (r.id === replyId ? { ...r, likes: r.likes + 1 } : r)) }
                }
