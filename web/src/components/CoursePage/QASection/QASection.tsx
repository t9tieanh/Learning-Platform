  ...
  return (
    <div className='mx-auto p-6 space-y-6'>
      ...
      <div className='space-y-4 max-h-[600px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-muted-foreground/30 scrollbar-thumb-rounded-lg'>
        ...
        {!isLoading && comments.length === 0 && <p className='text-sm text-muted-foreground px-2'>Chưa có bình luận nào.</p>}
                    ...
                    <div className='ml-auto opacity-0 group-hover:opacity-100 transition-opacity'>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align='end'>
                          <DropdownMenuItem className='text-blue-500 hover:bg-blue-500 focus:bg-blue-500 hover:text-white transition-colors' onClick={() => startEdit(comment.id, false, comment.content)}>
                            <Pencil className='w-4 h-4' /> Chỉnh sửa
                          </DropdownMenuItem>
                          <DropdownMenuItem className='text-red-500 hover:bg-red-500 focus:bg-red-500 hover:text-white transition-colors' onClick={() => askDelete(comment.id)}>
                ...
                {editingTarget && editingTarget.id === comment.id && !editingTarget.isReply ? (
                  <div className='space-y-2'>
                    ...
                    <div className='flex gap-2 justify-end'>
                      <Button variant='outline' size='sm' onClick={cancelEdit} disabled={isEditingSaving} className='border-red-500 text-red-500 hover:bg-red-500 hover:text-white'>
                        Hủy
                      </Button>
                      {(() => {
                        const originalContent = comments.find((c: CommentUI) => c.id === editingTarget.id)?.content || ''
                        ...
                        {userId && userId === reply.userId && (
                          <div className='ml-auto opacity-0 group-hover:opacity-100 transition-opacity'>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align='end'>
                                <DropdownMenuItem className='text-blue-500 hover:bg-blue-500 focus:bg-blue-500' onClick={() => startEdit(reply.id, true, reply.content, comment.id)}>
                                  <Pencil className='w-4 h-4' /> Chỉnh sửa
                                </DropdownMenuItem>
                                <DropdownMenuItem className='text-red-500 hover:bg-red-500 focus:bg-red-500' onClick={() => askDelete(reply.id, true, comment.id)}>
                                  <Trash2 className='w-4 h-4' /> Xóa
                                </DropdownMenuItem>
                              </DropdownMenuContent>
                            </DropdownMenu>
                        ...
                        <div className='space-y-2'>
                          ...
                          <div className='flex gap-2 justify-end'>
                            ...
                            {(() => {
                              ...
                              const originalContent = parent?.replies.find((r: ReplyUI) => r.id === editingTarget.id)?.content || ''
                              const unchanged = originalContent.trim() === editContent.trim()
                              return (
                                <Button
                                  size='sm'
            ...
            {replyingTo === comment.id && (
              <div className='ml-14 space-y-3 pt-4 border-t border-border'>
                ...
                <div className='flex justify-end gap-2'>
                  ...
                  <Button size='sm' onClick={() => handleAddReply(comment.id)} disabled={!replyContent.trim() || isSubmitting}>
                    Trả lời
                  </Button>
                </div>
              </div>
